import { Alert, CodeGroup, Link, Presenter, Table } from '@/components/docs'

# Telefunc docs maintainer guide

<Alert type="warning" heading="Content-only workflow">
  Work in the MDX content files under `pages/(docs)/(content)/**`. Reuse the components exported from `@/components/docs` and avoid changing TSX, layout, routing, or theme files unless a developer explicitly asks for it.
</Alert>
...
<Table data={{
  ...
  rows: [
    ...
    ['components/docs/index.tsx', 'Whitelisted MDX component exports', 'Import only from here'],
...
<Presenter elevated>
   **Rule of thumb:** if the change is prose, examples, headings, links, screenshots, callouts, or tables, it belongs in MDX. If it changes routing, layout, styling, navigation wiring, or React code, hand it back to a developer.
</Presenter>
...
<Presenter elevated>
  <p>
    <strong>Safe editing pattern:</strong> first create or update the metadata entry in <code>lib/headings.ts</code>, then add that key to the right <code>links</code> array in <code>lib/navigation/menuNavigation.ts</code>.
  </p>
</Presenter>
...
<Table data={{
  ...
  rows: [
    ...
    ['Presenter', 'Framed screenshots, diagrams, or embedded visual examples', '<Presenter elevated>...</Presenter>'],
