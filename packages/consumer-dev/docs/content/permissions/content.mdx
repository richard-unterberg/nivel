...
```js title="TodoItem.telefunc.js" env="server"
...
```jsx title="TodoItem.jsx" env="client"
/** @param {string} text */
...
```ts title="auth/getUser.ts" env="server"
import { getContext, Abort } from 'telefunc'
...
```ts title="components/TodoItem.telefunc.ts" env="server"
